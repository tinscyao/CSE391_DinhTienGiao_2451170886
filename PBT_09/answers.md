...
## Câu C1 — Debug DOM Code (7 lỗi)

### Lỗi 1: `addEventListener("onclick"` → sai syntax
```javascript
// SAI
...
### Lỗi 2: `countDisplay = count` → thiếu property
```javascript
// SAI
countDisplay = count;
...
### Lỗi 5: localStorage trả về string, cần parse
```javascript
// SAI
...
### Lỗi 6: Decrement không thêm history
```javascript
// THIẾU - cần thêm code lưu history giống increment
```

### Lỗi 7: Không kiểm tra null khi load
```javascript
// SAI
count = localStorage.getItem("count");
